...
public class XhAiStreamClient {
    ...
    public String getAuthUrl() {
        try {
            ...
            String authorizationOrigin = String.format(
                    ...
                    "headers=\"%s\", " +
                    "signature=\"%s\"",
                    xfXhConfig.getApiKey(), "hmac-sha256", "host date request-line", sha);
            // 拼接地址
    ...
    public RequestDTO getRequestParam(String uid, List<MsgDTO> msgList) {
        ...
        requestDTO.setParameter(new RequestDTO.ParameterDTO(
                new RequestDTO.ParameterDTO.ChatDTO(
                        xfXhConfig.getDomain(),
                        xfXhConfig.getTemperature(),
                        xfXhConfig.getMaxTokens())));
        requestDTO.setPayload(new RequestDTO.PayloadDTO(
                new RequestDTO.PayloadDTO.MessageDTO(msgList)));
        return requestDTO;
    }
